...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form_obj.is_valid():
            # print(form_obj.cleaned_data)  # {'username': 'mmp', 'email': '22334@qq.com',
            # 'confirm_password': '1234', 'password': '1234'}
            clean_data = form_obj.cleaned_data  # 将校验的数据字段赋值给一个变量
            # 将字典里的confirm_password键值对删除
            clean_data.pop('confirm_password')  # {'username': 'mmp', 'email': '22334@qq.com',
            #  'password': '1234'}
            # 用户头像
...
def login(request):
    if request.method == 'POST':
        ...
        # 1.先校验验证码是否正确  自己决定是忽略   统一转大写或者小写
...
'''
内存管理器模块
BytesIO: 临时帮你存储数据，返回的时候的数据是二进制
StringIO: 临时帮你存储数据，返回的时候的数据是字符串
'''
def get_code(request):
    # 推导步骤一：直接获取后端现成的二进制数据发送给后端
    # with open(r'static\img\222.png', 'rb') as f:
    #     data = f.read()
    # return HttpResponse(data)

    # 推导步骤二： 利用pillow模块动态的产生图片
    # img_obj = Image.new('RGB', (200, 38), get_random())
    # # 先将图片对象保存起来
    ...
    # 推导步骤三：解决 文件存储繁琐，io操作效率低， 借助于内存管理模块
    # img_obj = Image.new('RGB', (200, 38), get_random())
    ...
    img_font = ImageFont.truetype('static/font/1.ttf', 30)  # 字体样式大小
    ...
    for i in range(5):
        ...
        code += tmp
    print(code)
    # 随机验证码在登陆的视图函数里面要用到,要比对，所以要找地方存起来，并且其它的视图函数也能够拿到
...
def site(request, username, **kwargs):
    ...
    # 如果用户名不存在那么返回一个404 页面
    if not user_obj:
        return render(request, 'error.html')
    blog = user_obj.blog
    # 查询当前个人站点下的所有的文章
    article_list = models.Article.objects.filter(blog=blog)  # 侧边栏的筛选其实就是对article_list再进一步的筛选 queryset对象
    if kwargs:
    ...
    #     count_num=Count('article__pk')).values_list(
    #     'name', 'count_num', 'pk')
    # # print(category_list)
    # # 查询当前用户所有的标签及标签下的文章数
...
def up_or_down(request):
    ...
    if request.is_ajax():
        ...
        if request.user.is_authenticated():
            ...
            is_up = json.loads(is_up)  # 将json内的字符串"true" 转成python内的布尔值True
            # print(is_up, type(is_up))
            # 2.判断当前用户是否是当前用户自己写的
            # 根据文章id 查询文章对象 根据文章对象查作者，跟request.user比对
            article_obj = models.Article.objects.filter(pk=article_id).first()
            if not article_obj.blog.userinfo == request.user:
                # 3.校验当前用户是否己经点了  哪个地方记录了用户点还是没点
                is_click = models.UpAndDown.objects.filter(user=request.user, article=article_obj)
                if not is_click:
                    # 4.操作数据库 记录数据  要同步操作普通字段
                    # 判断当前用户是点了赞还是点了踩 从而决定给哪个字段加一
                    if is_up:
                        # 给点赞数加一
                    ...
                    back_dic['code'] = 1001
                    back_dic['msg'] = '你己经点过了，不能再点了'  # 这里可以做的更加的详细，提示用户到底点了赞还是点了踩。
            else:
                back_dic['code'] = 1002
                back_dic['msg'] = '你不能够为自己点赞'
        else:
            back_dic['code'] = 1003
...
def upload_image(request):
    ...
    :return:
    """
    back_dic = {'error': 0}  # 先提前定义好返回给编辑器的数据格式
    # 用户写文章上传的图片， 也算静态资源， 也应该放到media文件下
    if request.method == 'POST':
        # 获取用户上传的图片对象
        # print(request.FILES)
        # <MultiValueDict: {'imgFile': [<InMemoryUploadedFile: 333.png (image/png)>]}>
        ...
        with open(file_path, 'wb') as f:
            for line in file_obj:
                f.write(line)
        # 为什么不直接使用file_path 因为file_path 是这样的 bbs项目/media/  没用开设接口
